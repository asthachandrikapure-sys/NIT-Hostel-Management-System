...
// Protect page
if(!isset($_SESSION['username']) || !in_array($_SESSION['role'], ["incharge_poly", "incharge_engg", "principal_poly", "principal_engg"])){
...
$pending_query = "SELECT g.id, g.student_name, g.reason, g.leave_date, g.return_date, g.created_at, g.remarks, s.academic_year, s.department, s.parent_mobile 
                  ...
                  WHERE g.status = 'Pending Hostel In-Charge Approval' AND s.college_name = '$college'
...
$history_query = "SELECT g.id, g.student_name, g.reason, g.status, g.created_at, g.remarks, s.academic_year, s.department 
                  FROM gate_passes g 
                  JOIN students_info s ON g.user_id = s.user_id 
                  WHERE g.status NOT IN ('Pending HOD Approval', 'Pending Hostel In-Charge Approval') AND s.college_name = '$college'
...
<head>
    ...
    <title>Gate Pass Approvals - College Admin</title>
    <link rel="stylesheet" href="college_admin.css">
    <style>
...
<body>
    ...
    <div class="sidebar-overlay" id="sidebarOverlay"></div>
        ...
        <div class="sidebar">
            <h2>Modules</h2>
            <a href="javascript:history.back()">Dashboard Home</a>
            <a href="college_admin_gatepass.php" style="background: #e7f3ea;">Gate Pass Approvals</a>
            <a href="principal_student_info.php">Student List</a>
        </div>

        <div class="content">
            <h1>Gate Pass Management</h1>
            <p>Level 2 Review for <strong><?php echo $college; ?></strong>.</p>
                            ...
                            <?php while($row = $pending_result->fetch_assoc()): ?>
                                <tr>
                                    <td>
                                        ...
                                        <small><?php echo htmlspecialchars($row['department_name']); ?> (<?php echo htmlspecialchars($row['academic_year']); ?>)</small>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="remark-box"><?php echo htmlspecialchars($row['remarks'] ?? 'No remarks'); ?></div>
                                    </td>
                                    <td>
                                        <form method="POST" action="" style="display:flex; flex-direction:column; gap:8px;" onsubmit="return validateRemark(this);">
                                            ...
                                            <textarea name="remark" placeholder="Admin remark..." style="padding:4px; border-radius:4px; border:1px solid #ccc; font-size:11px;" oninput="toggleButtons(this)"></textarea>
